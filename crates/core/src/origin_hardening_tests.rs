...
#[test]
fn converts_the_exact_verified_origin_9_profile() {
    let mut project = project(vec![worksheet("Sheet", columns(&["value"]))]);
    project.probe = OriginProbe {
        format: OriginFormat::Opj,
        raw_version: "4.3268 195 W64".to_owned(),
        version: OriginHeaderVersion {
            major: 4,
            minor: 3268,
            build: 195,
        ...
        byte_order: OriginByteOrder::LittleEndian,
        profile: Some(OriginProfile::Origin9V951),
        support: OriginSupport::Supported,
    };
    ...
    .expect("the verified Origin 9.51 profile should convert");

    assert_eq!(imported[0].snapshot.schema.columns[0].name, "value");
}
