...
fn validate_probe(project: &OriginProject) -> Result<(), OriginImportError> {
    const ORIGIN_7_VERSION: OriginHeaderVersion = OriginHeaderVersion {
        major: 4,
        minor: 2673,
        build: 552,
    };
    const ORIGIN_9_VERSION: OriginHeaderVersion = OriginHeaderVersion {
        major: 4,
        minor: 3268,
        build: 195,
    };
    let exact_profile = match project.probe.profile {
        Some(OriginProfile::Origin7V552) => {
            project.probe.raw_version == "4.2673 552" && project.probe.version == ORIGIN_7_VERSION
        }
        Some(OriginProfile::Origin9V951) => {
            project.probe.raw_version == "4.3268 195 W64"
                && project.probe.version == ORIGIN_9_VERSION
        }
        None => false,
    };
        ...
        || !exact_profile
    {
        return Err(OriginImportError::InvalidModel {
            detail:
                "only the exact verified little-endian Origin7V552 or Origin9V951 OPJ profile can be converted"
                    .to_owned(),
        });
    }
    Ok(())
