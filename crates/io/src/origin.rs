...
const ORIGIN_9_V951_VERSION: &str = "4.3268 195 W64";

/// Maximum leading-byte prefix needed to validate every supported Origin profile.
///
/// Bounded format dispatchers should read this many bytes, or the complete file
/// when it is shorter, before calling [`probe_origin`].
pub const MAX_PROBE_BYTES: usize = 152;
...
pub enum OriginProfile {
    ...
    Origin7V552,
    /// 64-bit Origin 9.51 project header `CPYA 4.3268 195 W64 #`.
    Origin9V951,
}
...
fn probe_origin_with_limits(
    ...
    let raw_version = match format {
        OriginFormat::Opj => {
            ...
            value
                .strip_suffix(' ')
                .filter(|value| value.ends_with(" W64"))
                .unwrap_or(value)
        }
    ...
    let version = parse_version(raw_version, format)?;
    let (raw_version, retained_parser_bytes) =
        copy_header_version(raw_version, limits, initial_total_owned_bytes)?;

    match format {
        OriginFormat::Opj => {
            let profile = match raw_version.as_str() {
                ORIGIN_7_V552_VERSION => OriginProfile::Origin7V552,
                ORIGIN_9_V951_VERSION => OriginProfile::Origin9V951,
                _ => return Err(OriginError::UnsupportedVersion { raw_version }),
            };
            opj::validate_initial_structure(bytes, profile, limits.max_block_bytes)?;
...
fn parse_version(
    raw_version: &str,
    format: OriginFormat,
) -> Result<OriginHeaderVersion, OriginError> {
    let mut fields = raw_version.split(' ');
    let version = fields.next().unwrap_or_default();
    let build = fields.next().unwrap_or_default();
    let platform = fields.next();
    let platform_is_valid =
        platform.is_none() || matches!((format, platform), (OriginFormat::Opj, Some("W64")));
    if version.is_empty() || build.is_empty() || !platform_is_valid || fields.next().is_some() {
        return malformed(
            "the header must contain version and numeric build fields followed only by the optional classic OPJ W64 marker",
        );
    }
