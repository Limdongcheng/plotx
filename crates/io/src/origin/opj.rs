...
use super::{
    OriginColumn, OriginDiagnosticCode, OriginError, OriginLimits, OriginProbe, OriginProfile,
    OriginProject, OriginResourceUsage, OriginWorkbook, OriginWorksheet,
};
...
struct ProfileLayout {
    name: &'static str,
    raw_version: &'static str,
    signature: &'static [u8],
    global_header_len: usize,
    embedded_version: f64,
    data_header_len: usize,
    metadata_policy: MetadataPolicy,
}

const ORIGIN7_LAYOUT: ProfileLayout = ProfileLayout {
    name: "Origin7V552",
    raw_version: "4.2673 552",
    signature: b"CPYA 4.2673 552#\n",
    global_header_len: 39,
    embedded_version: 7.0552,
    data_header_len: 123,
    metadata_policy: MetadataPolicy::Full,
};

const ORIGIN9_LAYOUT: ProfileLayout = ProfileLayout {
    name: "Origin9V951",
    raw_version: "4.3268 195 W64",
    signature: b"CPYA 4.3268 195 W64 #\n",
    global_header_len: 115,
    embedded_version: 9.510195,
    data_header_len: 147,
    metadata_policy: MetadataPolicy::WindowsOnly,
};

fn profile_layout(profile: OriginProfile) -> &'static ProfileLayout {
    match profile {
        OriginProfile::Origin7V552 => &ORIGIN7_LAYOUT,
...
pub(super) fn validate_initial_structure(
    ...
    let initial_structure_len = layout
        .signature
        .len()
        .checked_add(BLOCK_PREFIX_LEN)
        .and_then(|length| length.checked_add(layout.global_header_len))
        .and_then(|length| length.checked_add(1 + BLOCK_PREFIX_LEN))
    ...
    let signature = reader.read_slice(layout.signature.len())?;
    ...
    require_exact_length(
        header_offset,
        header,
        layout.global_header_len,
        "Origin header payload",
    )?;
    validate_embedded_origin_version(header_offset, header, layout)?;
...
pub(super) fn read(
    ...
    let profile = probe
        .profile
        .ok_or_else(|| OriginError::UnsupportedVersion {
            raw_version: probe.raw_version.clone(),
        })?;
    let raw = parse_raw(bytes, limits, profile, retained_probe_bytes)?;
    ...
    let mut parsed = match profile_layout(profile).metadata_policy {
        MetadataPolicy::Full => {
            metadata::parse(raw.remaining, metadata_offset, limits, &mut usage)?
        }
        MetadataPolicy::WindowsOnly => {
            metadata::parse_windows_only(raw.remaining, metadata_offset, limits, &mut usage)?
        }
    };
    let mut unsupported_columns = 0_usize;

    for section in &raw.data_sections {
        match records::decode_column_record(
            profile,
            section.header,
            section.content,
            limits,
...
pub(super) fn parse_raw<'a>(
    ...
    if signature != layout.signature {
        return Err(OriginError::UnsupportedVersion {
            raw_version: format!("classic OPJ signature does not match {}", layout.name),
        });
    }
    ...
    validate_embedded_origin_version(origin_header_offset, origin_header, layout)?;
    ...
    loop {
        // Each verified profile's data list is a sequence of
        // <profile-sized header block><content block or null><null block>,
        // followed by a consumed list null. Parsing stops at that exact
        // bounded boundary.
        // https://github.com/jgonera/openopj/blob/42ddcf1eb3a490744c54fca0a4ed6fe7a5e723ca/docs/opj_format.markdown
        // https://github.com/jgonera/openopj/blob/42ddcf1eb3a490744c54fca0a4ed6fe7a5e723ca/lib/OpenOPJ/OPJFile.php
        let header_block = reader.read_block()?;
        ...
        require_exact_length(
            header_offset,
            header,
            layout.data_header_len,
            match profile {
                OriginProfile::Origin7V552 => "Origin7V552 data-header payload",
                OriginProfile::Origin9V951 => "Origin9V951 data-header payload",
            },
        )?;
...
fn validate_embedded_origin_version(
    ...
    if version.to_bits() != layout.embedded_version.to_bits() {
        return Err(OriginError::UnsupportedVersion {
            raw_version: format!("{} header embeds Origin {version}", layout.raw_version),
        });
    }
    Ok(())
}
