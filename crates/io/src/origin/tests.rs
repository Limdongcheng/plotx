...
#[test]
fn classic_w64_header_is_recognized_as_an_unsupported_version() {
    assert_eq!(
        probe_origin(b"CPYA 4.3224 220 W64 #\n").unwrap_err(),
        OriginError::UnsupportedVersion {
            raw_version: "4.3224 220 W64".to_owned(),
        }
    );
}
...
mod origin9_profile {
    ...
    const ORIGIN_HEADER_LEN: usize = 115;

    fn initial_structure(header_len: usize, embedded_version: f64) -> Vec<u8> {
        let mut bytes = SIGNATURE.to_vec();
        bytes.extend_from_slice(&u32::try_from(header_len).unwrap().to_le_bytes());
        bytes.push(b'\n');
        let mut header = vec![0; header_len];
        header[0x1b..0x23].copy_from_slice(&embedded_version.to_le_bytes());
        bytes.extend_from_slice(&header);
        bytes.push(b'\n');
        bytes.extend_from_slice(&[0, 0, 0, 0, b'\n']);
        bytes
    ...
    fn assert_structured_probe_error(bytes: &[u8]) {
        let result = std::panic::catch_unwind(|| probe_origin(bytes));
        let probe = result.expect("probing an invalid Origin 9 prefix must not panic");
        if let Ok(probe) = probe {
            assert_ne!(probe.profile, Some(OriginProfile::Origin7V552));
            panic!("an invalid Origin 9 prefix must return a structured error");
        }
    }

    #[test]
    fn recognizes_exact_origin9_profile() {
        let bytes = initial_structure(ORIGIN_HEADER_LEN, 9.510195);

        let probe = probe_origin(&bytes).unwrap();

        assert_eq!(probe.profile, Some(OriginProfile::Origin9V951));
        assert_eq!(probe.support, OriginSupport::Supported);
        assert_eq!(probe.raw_version, "4.3268 195 W64");
    }

    #[test]
    fn rejects_114_byte_global_header() {
        assert_structured_probe_error(&initial_structure(114, 9.510195));
    }

    #[test]
    fn rejects_116_byte_global_header() {
        assert_structured_probe_error(&initial_structure(116, 9.510195));
    }

    #[test]
    fn rejects_changed_embedded_version() {
        assert_structured_probe_error(&initial_structure(ORIGIN_HEADER_LEN, 9.510194));
    }

    #[test]
    fn rejects_every_truncated_initial_structure_prefix() {
        let bytes = initial_structure(ORIGIN_HEADER_LEN, 9.510195);
        for end in 0..bytes.len() {
            assert_structured_probe_error(&bytes[..end]);
        }
    }
}
