...
pub(super) fn decode_column_record(
    ...
    require_header_length(profile, header)?;
...
fn require_header_length(profile: OriginProfile, header: &[u8]) -> Result<(), OriginError> {
    let expected = match profile {
        OriginProfile::Origin7V552 => HEADER_LEN,
        OriginProfile::Origin9V951 => 147,
    };
    if header.len() < expected {
        return Err(OriginError::Truncated {
            offset: header.len(),
            needed: expected - header.len(),
            have: 0,
        });
    }
    if header.len() > expected {
        return Err(OriginError::CorruptStructure {
            offset: expected,
            detail: format!(
                "{} data header must be exactly {expected} bytes, not {}",
...
fn classify_type(
    ...
    if profile == OriginProfile::Origin9V951 {
        return classify_modern_type(
            data_type,
            secondary,
            width,
            unsigned_flag,
...
fn classify_modern_type(
    ...
    // Match complete tuples from two Origin 9.51 projects. Checking fields
    // independently would silently accept an unobserved Cartesian product.
    let verified = matches!(
        ...
        (0x5121, 0x03, 10, 0x20, 0x10c8, false)
            | (0x5121, 0x03, 10, 0x30, 0x10c8, false)
            | (0x5121, 0x03, 10, 0x30, 0x10ca, false)
            | (0x6121, 0x03, 10, 0x00, 0x11ca, false)
            | (0x6121, 0x03, 10, 0x10, 0x10c8, false)
            | (0x6121, 0x03, 10, 0x10, 0x10c9, false)
            | (0x6121, 0x03, 10, 0x10, 0x10ca, false)
            | (0x6121, 0x01, 10, 0x00, 0x10ca, true)
            | (0x6121, 0x03, 10, 0x00, 0x10ca, true)
    );
    if !verified {
        return unsupported("the complete Origin9V951 dataset type tuple is not verified");
    }
    Ok(ValueKind::ModernF64)
}

fn profile_name(profile: OriginProfile) -> &'static str {
    match profile {
        OriginProfile::Origin7V552 => "Origin7V552",
        OriginProfile::Origin9V951 => "Origin9V951",
    }
}
...
fn decode_cell(
    ...
    match kind {
        ...
        ValueKind::ModernF64 => decode_modern_f64(slot, offset),
    }
}

fn decode_modern_f64(slot: &[u8], offset: usize) -> Result<OriginCell, OriginError> {
    let discriminator = *slot.first().ok_or_else(|| truncated_range(offset, 1, 0))?;
    let reserved = *slot.get(1).ok_or_else(|| truncated_range(offset, 2, 1))?;
    if reserved != 0 {
        return Err(OriginError::CorruptStructure {
            offset: checked_add(offset, 1, "Origin9V951 reserved prefix offset")?,
            detail: "the reserved Origin9V951 numeric prefix byte must be zero".to_owned(),
        });
    }
    if discriminator != 0 {
        return unsupported("Origin9V951 text and unknown cell discriminators are not supported");
    }
    let payload = slot
        .get(2..)
    ...
    decode_f64(
        payload,
        checked_add(offset, 2, "Origin9V951 numeric value offset")?,
    )
}
...
#[cfg(test)]
#[path = "records_modern_tests.rs"]
mod records_modern_tests;
