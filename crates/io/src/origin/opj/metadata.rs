...
pub(super) fn parse_windows_only(
    bytes: &[u8],
    base_offset: usize,
    limits: &OriginLimits,
    usage: &mut OriginResourceUsage,
    ...
    // The Origin9V951 evidence establishes the complete window-list boundary,
    // but not the object grammar that follows it. Stop at the consumed null
    // terminator and report one opaque tail instead of scanning for markers.
    if cursor.remaining() > 0 {
        push_summary(
            &mut unsupported_objects,
            "remaining Origin 9.51 project objects",
            1,
            limits,
            usage,
        )?;
        push_diagnostic(
            &mut diagnostics,
            OriginDiagnosticCode::UnsupportedObjectSkipped,
            "PlotX imported worksheet values but did not import the remaining Origin 9.51 project objects.",
            Some(cursor.absolute_offset()?),
            limits,
            usage,
        )?;
    }
...
#[cfg(test)]
#[path = "metadata_modern_tests.rs"]
mod metadata_modern_tests;
